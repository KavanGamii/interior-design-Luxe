...
const socials = config?.socials || {};
    ...
    const address = config?.contactAddress || "Worli Sea Face, Mumbai";
            ...
            <div className="md:col-span-3 footer-reveal pt-8 md:pt-16">
              ...
              <div className="space-y-8">
                <div>
                  ...
                  <a href={`mailto:${email}`} className="text-xl font-serif hover:text-accent-gold border-b border-cream/10 pb-1">{email}</a>
                </div>
                <div>
                  <p className="text-xs text-cream/40 uppercase tracking-widest mb-2 font-black">Studio HQ</p>
                  <p className="text-xl font-serif text-cream/80 whitespace-pre-line">{address}</p>
                </div>
                <div>
              ...
              {[...Array(4)].map((_, i) => (
                <div key={i} className="flex gap-24 pr-24">
                  {Object.entries(socials).map(([key, value]) => (
                    <a key={key} href={value as string} target="_blank" className="hover:text-accent-gold transition-colors capitalize">{key}</a>
                  ))}
                </div>
              ))}
            </div>
